...
TEST(UnnecessaryNullCheckForCXXDeallocRuleTest, Comparator) {
    ...
    const char *code2 = "void m(char* c) { " VIOLATION_START "if (c != (void*)0) delete " VIOLATION_END
                        "c; }";
    const char *code3 = "void m(char* c) { " VIOLATION_START "if (c != nullptr) delete " VIOLATION_END
                        "c; }";
...
TEST(UnnecessaryNullCheckForCXXDeallocRuleTest, ResetDeleteBlock) {
    const char *code1 = "void m(char* c) { " VIOLATION_START "if (c) { delete c; c = 0; " VIOLATION_END
                        "} }";
    const char *code2 = "void m(char* c) { " VIOLATION_START "if (c) { delete c; c = nullptr; "
                        VIOLATION_END "} }";
...
TEST(UnnecessaryNullCheckForCXXDeallocRuleTest, DeleteArray) {
    const char *code1 = "void m(char* c) { " VIOLATION_START "if (c != 0) delete [] " VIOLATION_END
                        "c; }";
    const char *code2 = "struct S {~S();}; void m(S* s) { " VIOLATION_START "if (s) delete[]"
                        VIOLATION_END "s;}";
