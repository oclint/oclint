...
TEST(EmptyCatchStatementRuleTest, MultipleEmptyCXXCatchStmts) {
    testRuleOnCXXCode(new EmptyCatchStatementRule(),
                      "void m() { try { ; } catch (int i) {} catch(...) {} }",
                      0, 1, 36, 1, 37);
    testRuleOnCXXCode(new EmptyCatchStatementRule(),
                      "void m() { try { ; } catch (int i) {} catch(...) {} }",
                      1, 1, 50, 1, 51);
...
TEST(EmptyCatchStatementRuleTest, MultipleEmptyObjCCatchStmts) {
    testRuleOnObjCCode(new EmptyCatchStatementRule(),
                       "void m() { @try { ; } @catch(id i) {} @catch(id ex) {} }",
                       0, 1, 36, 1, 37);
    testRuleOnObjCCode(new EmptyCatchStatementRule(),
                       "void m() { @try { ; } @catch(id i) {} @catch(id ex) {} }",
                       1, 1, 53, 1, 54);
