...
TEST(JumbledIncrementerRuleTest, GoodNestedForLoops) {
    testRuleOnCode(new JumbledIncrementerRule(),
                   "void aMethod(int a) { for (int i = 0; i < a; i++) { for (int j = 0; j < a; j++) {;} } }");
}

TEST(JumbledIncrementerRuleTest, InnerForLoopInsideCompoundStmt) {
    testRuleOnCode(new JumbledIncrementerRule(),
                   "void aMethod(int a) { for (int i = 0; i < a; i++) { for (int j = 0; j < a; i++) {;} } }",
                   0, 1, 76, 1, 77);
}
...
TEST(JumbledIncrementerRuleTest, MultiIncsAreFine) {
    testRuleOnCode(new JumbledIncrementerRule(),
                   "void aMethod(int a) { for (int i = 0; i < a; i++) for (int j = 0; j < a; i++, j++) {;} }");
}
