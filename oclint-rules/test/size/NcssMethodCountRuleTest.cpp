...
TEST_F(NcssMethodCountRuleTest, CXXTryCatch) {
    testRuleOnCXXCode(new NcssMethodCountRule(),
                      "void m() { try { int a = 1; } catch (...) { int b = 2; } }",
...
TEST_F(NcssMethodCountRuleTest, ObjCTryCatchFinally) {
    testRuleOnObjCCode(new NcssMethodCountRule(),
...
TEST_F(NcssMethodCountRuleTest, ObjCSynchronized) {
    testRuleOnObjCCode(new NcssMethodCountRule(),
                       "void m() { id res; @synchronized(res) { int a = 1; } }",
...
TEST_F(NcssMethodCountRuleTest, TwoCaseStatements) {
    testRuleOnCode(new NcssMethodCountRule(),
                   "void m() { int i = 1; switch (i) { case 1: case 2: break; } }",
...
TEST_F(NcssMethodCountRuleTest, SupressHighNcssMethod) {
    testRuleOnCode(new NcssMethodCountRule(),
                   "void __attribute__((annotate(\"oclint:suppress[high ncss method]\"))) aMethod() { do {} while(1); }");
}
