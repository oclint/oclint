...
if [ $SUCCESS -eq 0 ]; then
    case `uname` in
        MINGW32*)
            cmake -G "MSYS Makefiles" -D LLVM_ROOT="$LLVM_BUILD" -D GOOGLETEST_SRC="$GOOGLE_TEST_SRC" -D GOOGLETEST_BUILD="$GOOGLE_TEST_BUILD" -D OCLINT_SOURCE_DIR="$OCLINT_CORE_SRC" -D OCLINT_BUILD_DIR="$OCLINT_CORE_BUILD" -D OCLINT_METRICS_SOURCE_DIR="$OCLINT_METRICS_SRC" -D OCLINT_METRICS_BUILD_DIR="$OCLINT_METRICS_BUILD" -D TEST_BUILD=1 "$OCLINT_RULES_SRC"
        ;;
        *)
