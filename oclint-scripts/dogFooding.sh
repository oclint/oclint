...
OCLINT_DOGFOODING_DRIVER="$OCLINT_DOGFOODING/oclint-driver"
...
if [ $# -eq 1 ] && [ "$1" = "show" ]; then
    ...
    cat $OCLINT_DOGFOODING/dogfooding_driver_results.txt
    exit 0
fi
...
mkdir -p $OCLINT_DOGFOODING_DRIVER
...
cd $OCLINT_DOGFOODING_DRIVER
cmake -D CMAKE_EXPORT_COMPILE_COMMANDS=ON -D CMAKE_CXX_COMPILER=$LLVM_BUILD/bin/clang++ -D CMAKE_C_COMPILER=$LLVM_BUILD/bin/clang -D LLVM_ROOT=$LLVM_BUILD -D OCLINT_SOURCE_DIR=$OCLINT_CORE_SRC -D OCLINT_BUILD_DIR=$OCLINT_CORE_BUILD $OCLINT_DRIVER_SRC
...
cp $OCLINT_DOGFOODING_DRIVER/compile_commands.json $OCLINT_DRIVER_SRC/compile_commands.json
